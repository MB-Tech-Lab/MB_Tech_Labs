...
import { LayoutDashboard, FolderKanban, FileText, Receipt, Calendar, MessageSquare, Bell, Building2, LogOut, Menu, X, Loader2 } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { label: "Profile", href: "/client/profile", icon: Building2 },
];
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!tokenStorage.get()) { router.replace("/client/login"); return; }
  ...
  function isActive(href: string) { return href === "/client" ? pathname === "/client" : pathname.startsWith(href); }
    ...
    <div className="min-h-screen flex" style={{ background: "#0a0e1a", color: "#e2e8f0" }}>
      <aside className="hidden lg:flex flex-col sticky top-0 h-screen w-[240px] shrink-0 border-r" style={{ background: "#0f1428", borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-2.5 h-16 px-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-cyan/15 border border-cyan/30 text-cyan"><Building2 className="h-5 w-5" /></span>
          <div><p className="font-display font-bold text-[13px] text-white">Client Portal</p><p className="text-[9px] text-cyan-soft/60 uppercase tracking-wider">MB Tech Labs</p></div>
        </div>
        ...
        <div className="p-2 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <button onClick={() => { tokenStorage.clear(); router.push("/client/login"); }} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12.5px] text-rose-300 hover:bg-rose-400/10"><LogOut className="h-4 w-4" /> Logout</button>
        </div>
      </aside>
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 lg:hidden">
            ...
            <motion.div initial={{ x: -240 }} animate={{ x: 0 }} exit={{ x: -240 }} className="absolute left-0 top-0 bottom-0 w-[240px] flex flex-col" style={{ background: "#0f1428" }}>
              <div className="flex items-center justify-between h-16 px-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                <span className="font-display font-bold text-[13px] text-white">Client Portal</span>
              ...
              <nav className="flex-1 py-3 px-2 space-y-0.5">
                {NAV_ITEMS.map((item) => {
                  ...
                  const active = isActive(item.href);
                  return (
                    <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={cn("flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12.5px] font-medium", active ? "bg-cyan/10 text-cyan" : "text-white/55 hover:bg-white/[0.04]")}>
              ...
              <div className="p-2 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                <button onClick={() => { tokenStorage.clear(); router.push("/client/login"); }} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12.5px] text-rose-300"><LogOut className="h-4 w-4" /> Logout</button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <div className="flex-1 min-w-0 flex flex-col h-screen">
        <header className="h-14 shrink-0 flex items-center gap-3 px-4 border-b" style={{ background: "#0f1428", borderColor: "rgba(255,255,255,0.06)" }}>
          ...
          <span className="text-[13px] text-white/50">Client Portal</span>
          <div className="ml-auto"><span className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-cyan/15 border border-cyan/30 font-display font-semibold text-[12px] text-cyan-soft">CL</span></div>
        </header>
